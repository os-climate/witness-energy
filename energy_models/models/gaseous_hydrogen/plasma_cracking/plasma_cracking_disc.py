...
class PlasmaCrackingDiscipline(GaseousHydrogenTechnoDiscipline):
   ...
   GRADIENT H2 VS INVEST_LEVEL
        ...
        dcarbon_prod_dinvest = self.dprod_column_dinvest[f"{GlossaryEnergy.CarbonResource} (Mt)"] * \
                               scaling_factor_invest_level  # / scaling_factor_techno_production
   ...
   GRADIENT H2 VS ALL_DEMAND_RATIO
        ...
        if GlossaryEnergy.electricity in self.dprod_column_dratio[f"{GlossaryEnergy.CarbonResource} (Mt)"].keys():
            dcarbon_prod_dratio_elec = self.dprod_column_dratio[f"{GlossaryEnergy.CarbonResource} (Mt)"][
                                           GlossaryEnergy.electricity] * \
                                       scaling_factor_invest_level  # / scaling_factor_techno_production
        else:
            dcarbon_prod_dratio_elec = np.array([[0] * len(self.dprod_dratio[list(self.dprod_dratio.keys())[
                0]][0])] * len(self.dprod_dratio[list(self.dprod_dratio.keys())[0]][0]))
        if GlossaryEnergy.methane in self.dprod_column_dratio[f"{GlossaryEnergy.CarbonResource} (Mt)"].keys():
            dcarbon_prod_dratio_meth = self.dprod_column_dratio[f"{GlossaryEnergy.CarbonResource} (Mt)"][GlossaryEnergy.methane] * \
                                       scaling_factor_invest_level  # / scaling_factor_techno_production
        else:
