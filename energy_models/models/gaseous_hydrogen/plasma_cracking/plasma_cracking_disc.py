...
class PlasmaCrackingDiscipline(GaseousHydrogenTechnoDiscipline):
    ...
    CO2_credits = pd.DataFrame({GlossaryCore.Years: range(2020, 2051),
                                'CO2_credits': 50.})

    market_demand = pd.DataFrame(
        {GlossaryCore.Years: range(2020, 2051), 'carbon_demand': 5e-2})
...
# DESC_IN.update(
    ...
    def setup_sos_disciplines(self):
        ...
        if self.get_data_in() is not None:
            if GlossaryCore.YearStart in self.get_data_in():
                ...
                if self.get_sosdisc_inputs('CO2_credits')[GlossaryCore.Years].values.tolist() != list(years):
                    self.update_default_value(
                        'CO2_credits', self.IO_TYPE_IN, pd.DataFrame({GlossaryCore.Years: years, 'CO2_credits': 50.}))

                if self.get_sosdisc_inputs('market_demand')[GlossaryCore.Years].values.tolist() != list(years):
                    self.update_default_value(
                        'market_demand', self.IO_TYPE_IN, pd.DataFrame({GlossaryCore.Years: years, 'carbon_demand': 5e-2}))
    ...
    def specific_run(self):
        ...
        outputs_dict = {GlossaryCore.TechnoPricesValue: techno_prices,
                        'percentage_resource': percentage_resource,
                        'carbon_quantity_to_be_stored': quantity[[GlossaryCore.Years, 'carbon_storage']]
                        # quantity[[GlossaryCore.Years, 'carbon_production', 'carbon_sales', 'carbon_storage']]
                        }
   ...
   GRADIENT H2 VS ENERGY_CO2_EMISSION
        ...
        energy_CO2_emission = self.get_sosdisc_inputs(GlossaryCore.EnergyCO2EmissionsValue)
            ...
            if energy != GlossaryCore.Years:
                ...
                if (dtechno_price_denergy_CO2_emission is not None):
                    ...
                    self.set_partial_derivative_for_other_types(
                        (GlossaryCore.TechnoPricesValue, self.techno_name), (GlossaryCore.EnergyCO2EmissionsValue, energy), value)
   ...
   GRADIENT H2 VS ENERGY_PRICES
        ...
        energy_prices = self.get_sosdisc_inputs(GlossaryCore.EnergyPricesValue)
        techno_prices = self.get_sosdisc_outputs(
            GlossaryCore.TechnoPricesValue)[self.techno_name].values
        techno_prices_wotaxes = self.get_sosdisc_outputs(
            GlossaryCore.TechnoPricesValue)[f'{self.techno_name}_wotaxes'].values
        ...
        for energy in energy_prices:
            if energy != GlossaryCore.Years:
                ...
                if dtechno_price_denergy_prices is not None:
                    ...
                    self.set_partial_derivative_for_other_types(
                        ('percentage_resource', 'hydrogen.gaseous_hydrogen'), (GlossaryCore.EnergyPricesValue, energy), dpercentage_resources_denergy_prices * 100)
                    self.set_partial_derivative_for_other_types(
                        (GlossaryCore.TechnoPricesValue, self.techno_name), (GlossaryCore.EnergyPricesValue, energy), value)
                    self.set_partial_derivative_for_other_types(
                        (GlossaryCore.TechnoPricesValue, f'{self.techno_name}_wotaxes'), (GlossaryCore.EnergyPricesValue, energy), value_wotaxes)
   ...
   GRADIENT H2 VS INVEST_LEVEL
        ...
        if (dhydro_prod_dinvest is not None) & (dcarbon_prod_dinvest is not None):
            ...
            self.set_partial_derivative_for_other_types(
                ('percentage_resource', 'hydrogen.gaseous_hydrogen'), (GlossaryCore.InvestLevelValue, GlossaryCore.InvestValue), dpercentage_resources_dinvest * 100)
        ...
        if (dhydro_prod_dinvest is not None) & (dcarbon_prod_dinvest is not None) & (dtechno_price_dinvest is not None) & (dtechno_price_dinvest_wotaxes is not None):
            ...
            self.set_partial_derivative_for_other_types(
                (GlossaryCore.TechnoPricesValue, self.techno_name), (GlossaryCore.InvestLevelValue, GlossaryCore.InvestValue), value)
            self.set_partial_derivative_for_other_types(
                (GlossaryCore.TechnoPricesValue, f'{self.techno_name}_wotaxes'), (GlossaryCore.InvestLevelValue, GlossaryCore.InvestValue), value_wotaxes)
   ...
   GRADIENT H2 VS CO2_TAXES
        ...
        techno_prices_techno = self.techno_model.cost_details[[
            GlossaryCore.Years, self.techno_name, f'{self.techno_name}_wotaxes']].merge(percentage_resource_df, how='left').fillna(0)
   ...
   GRADIENT H2 VS ALL_DEMAND_RATIO
        ...
        self.set_partial_derivative_for_other_types(
            ('percentage_resource', 'hydrogen.gaseous_hydrogen'), (GlossaryCore.AllStreamsDemandRatioValue, 'electricity'), dpercentage_resources_dratio_elec)
        self.set_partial_derivative_for_other_types(
            ('percentage_resource', 'hydrogen.gaseous_hydrogen'), (GlossaryCore.AllStreamsDemandRatioValue, 'methane'), dpercentage_resources_dratio_meth)
        ...
        self.set_partial_derivative_for_other_types(
            (GlossaryCore.TechnoPricesValue, self.techno_name), (GlossaryCore.AllStreamsDemandRatioValue, 'electricity'), value_elec / 100)
        self.set_partial_derivative_for_other_types(
            (GlossaryCore.TechnoPricesValue, f'{self.techno_name}_wotaxes'), (GlossaryCore.AllStreamsDemandRatioValue, 'electricity'), value_wotaxes_elec / 100)

        self.set_partial_derivative_for_other_types(
            (GlossaryCore.TechnoPricesValue, self.techno_name), (GlossaryCore.AllStreamsDemandRatioValue, 'methane'), value_meth / 100)
        self.set_partial_derivative_for_other_types(
            (GlossaryCore.TechnoPricesValue, f'{self.techno_name}_wotaxes'), (GlossaryCore.AllStreamsDemandRatioValue, 'methane'), value_wotaxes_meth / 100)
   ...
   GRADIENT H2 VS RESOURCES_PRICES
        ...
        self.set_partial_derivative_for_other_types(
            ('percentage_resource', 'hydrogen.gaseous_hydrogen'), (GlossaryCore.ResourcesPriceValue, 'carbon_resource'), dpercentage_resources_dresources_price * 100)
        x = np.array(
            [[x_i if x_i > 0.0 else 1e-6 for x_i in percentage_resource], ] * len(years)).transpose()
        y = np.array([techno_prices, ] * len(years)).transpose()
        y_wotaxes = np.array(
            [techno_prices_wotaxes, ] * len(years)).transpose()
        self.set_partial_derivative_for_other_types(
            (GlossaryCore.TechnoPricesValue, self.techno_name), (GlossaryCore.ResourcesPriceValue, 'carbon_resource'), dpercentage_resources_dresources_price * y / x)
        self.set_partial_derivative_for_other_types(
            (GlossaryCore.TechnoPricesValue, f'{self.techno_name}_wotaxes'), (GlossaryCore.ResourcesPriceValue, 'carbon_resource'), dpercentage_resources_dresources_price * y_wotaxes / x)
