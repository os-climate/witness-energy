...
class Nuclear(ElectricityTechno):
    ...
    def compute_other_primary_energy_costs(self):
        ...
        return self.cost_details[f'{self.URANIUM_RESOURCE_NAME}'] + self.cost_details[Water.name] + self.cost_details[
            'waste_disposal']
    ...
    def grad_price_vs_resources_price(self):
        ...
        init_grad = np.identity(
            len(self.years)) / efficiency.values[:, np.newaxis]
        return {
            Water.name: init_grad * water_needs,
            self.URANIUM_RESOURCE_NAME: init_grad * uranium_needs,
        }
