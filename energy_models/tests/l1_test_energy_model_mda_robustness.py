...
class TestMDARobustness(AbstractJacobianUnittest):
    ...
    def test_01_multi_scenario_perfos_execute(self):
        ...
        values_dict[1]['Test.EnergyMix.energy_prices'] = pd.DataFrame({'years': years, 'electricity': np.array([0.09, 0.08974117039450046, 0.08948672733558984,
        ...
        for column in energy_prices0:
            for value1, value2 in zip(list(energy_prices0[column].values), list(energy_prices1[column].values)):
                self.assertAlmostEqual(
                    abs(value1 - value2) / value1, 0.0, delta=100 * tolerance, msg=f'{column} : {value1} and {value2} are different')
