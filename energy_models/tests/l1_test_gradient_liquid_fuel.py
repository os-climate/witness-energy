...
class LiquidFuelJacobianCase(AbstractJacobianUnittest):
    ...
    def setUp(self):
        ...
        self.energy_carbon_emissions = pd.DataFrame(
            {GlossaryCore.Years: years, 'electricity': 0.0, 'syngas': 0.0, 'hydrogen.gaseous_hydrogen': 0.})
        ...
        self.invest_level_negative2 = pd.DataFrame({GlossaryCore.Years: years,
                                                    GlossaryCore.InvestValue: np.array([4435750000.0, 4522000000.0, 4608250000.0,
                                                                        4694500000.0, 4780750000.0, 4867000000.0,
                                                                        4969400000.0, 5071800000.0, 5174200000.0,
                                                                        5276600000.0, 5379000000.0, 5364700000.0,
    ...
    def test_04_transesterification_discipline_analytic_grad_negative_invest2(self):
        ...
        inputs_dict = {f'{self.name}.{GlossaryCore.YearEnd}': 2050,
                       f'{self.name}.{GlossaryCore.EnergyPricesValue}': self.energy_prices,
                       f'{self.name}.{GlossaryCore.EnergyCO2EmissionsValue}': self.energy_carbon_emissions,
                       f'{self.name}.{self.model_name}.{GlossaryCore.InvestLevelValue}': self.invest_level_negative2,
                       f'{self.name}.{GlossaryCore.CO2TaxesValue}': self.co2_taxes,
                       f'{self.name}.{GlossaryCore.TransportMarginValue}': self.margin,
                       f'{self.name}.{GlossaryCore.TransportCostValue}': self.transport,
                       f'{self.name}.{self.model_name}.{GlossaryCore.MarginValue}': self.margin,
    ...
    def test_05_liquid_fuel_discipline_jacobian(self):
        ...
        not_coupled_ouputs = [
            'energy_detailed_techno_prices', GlossaryCore.EnergyProductionDetailedValue]
