...
class FGCO2MembranesTestCase(unittest.TestCase):
    """
    CO2 Membranes prices test class
    """
    ...
    def test_01_compute_co2_membranes_price(self):
        ...
        inputs_dict = {GlossaryCore.YearStart: 2020,
                       ...
                       GlossaryCore.InvestmentBeforeYearStartValue: CO2MembranesDiscipline.invest_before_year_start,
        ...
        co2_membranes_model = CO2Membranes('Flue_gas_capture.CO2Membranes')
        co2_membranes_model.configure_parameters(inputs_dict)
        co2_membranes_model.configure_parameters_update(inputs_dict)
        price_details = co2_membranes_model.compute_price()
        ...
        plt.plot(price_details[GlossaryCore.Years],
                 price_details['Flue_gas_capture.CO2Membranes'], label='SoSTrades Total')

        plt.plot(price_details[GlossaryCore.Years], price_details['transport'],
                 label='SoSTrades Transport')

        plt.plot(price_details[GlossaryCore.Years], price_details['Flue_gas_capture.CO2Membranes_factory'],
                 label='SoSTrades Factory')
        plt.legend()
        plt.ylabel('Price ($/kWh)')

    def test_02_compute_co2_membranes_price_prod_consumption(self):
        ...
        co2_membranes_model.configure_parameters_update(inputs_dict)
        price_details = co2_membranes_model.compute_price()
        co2_membranes_model.compute_consumption_and_production()
