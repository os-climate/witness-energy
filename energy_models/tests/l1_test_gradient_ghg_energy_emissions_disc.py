...
class GHGEnergyEmissionsDiscJacobianTestCase(AbstractJacobianUnittest):
    ...
    def setUp(self):
        ...
        for i, energy in enumerate(self.energy_list):
            if energy == 'biomass_dry':
                ...
                self.CO2_per_use[f'{energy}'] = streams_outputs_dict[GlossaryEnergy.biogas]['CO2_per_use']['value']
                self.CH4_per_use[f'{energy}'] = streams_outputs_dict[GlossaryEnergy.biogas]['CH4_per_use']['value']
                self.N2O_per_use[f'{energy}'] = streams_outputs_dict[GlossaryEnergy.biogas]['N2O_per_use']['value']
    ...
    def test_02_GHGEnergy_emissions_discipline_energy_prod_cons_jacobian(self):
        ...
        coupled_inputs.extend(
            [f'{self.name}.{energy}.{GlossaryEnergy.EnergyConsumptionValue}' for energy in energy_list_wobiomass_dry])
    ...
    def test_04_GHGENergy_energy_production_detailed_jacobian(self):
        ...
        coupled_inputs = [
            f'{self.name}.{GlossaryEnergy.EnergyProductionDetailedValue}']
