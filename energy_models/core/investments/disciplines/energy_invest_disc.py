...
class InvestEnergyDiscipline(SoSWrapp):
        ...
        if GlossaryCore.energy_list in self.get_data_in():
            ...
            if energy_list is not None:
                for energy in energy_list:
                    dynamic_outputs[f'{energy}.{GlossaryCore.InvestLevelValue}'] = {
    ...
    def run(self):
        ...
        self.energy_model.set_energy_list(input_dict[GlossaryCore.energy_list])

        scaling_factor_energy_investment = input_dict['scaling_factor_energy_investment']
        scaled_energy_investment = pd.DataFrame(
            {GlossaryCore.Years: input_dict[GlossaryCore.EnergyInvestmentsValue][GlossaryCore.Years], GlossaryCore.EnergyInvestmentsValue: input_dict[GlossaryCore.EnergyInvestmentsValue][
        ...
        for energy in input_dict[GlossaryCore.energy_list]:
            output_dict[f'{energy}.{GlossaryCore.InvestLevelValue}'] = energy_invest_df[[
                GlossaryCore.Years, energy]].rename(columns={energy: GlossaryCore.InvestValue})
    ...
    def compute_sos_jacobian(self):
        ...
        norm_mix = compute_norm_mix(
            inputs_dict['invest_energy_mix'], inputs_dict[GlossaryCore.energy_list])

        for energy in inputs_dict[GlossaryCore.energy_list]:
            grad_energy = inputs_dict['invest_energy_mix'][energy].values / \
                          norm_mix.values
            self.set_partial_derivative_for_other_types(
                (f'{energy}.{GlossaryCore.InvestLevelValue}', GlossaryCore.InvestValue), (GlossaryCore.EnergyInvestmentsValue, GlossaryCore.EnergyInvestmentsValue),
            ...
            self.set_partial_derivative_for_other_types(
                (f'{energy}.{GlossaryCore.InvestLevelValue}', GlossaryCore.InvestValue), ('invest_energy_mix', energy),
                np.identity(len(years)) * grad_energy_mix[:, np.newaxis])
            for energy_other in inputs_dict[GlossaryCore.energy_list]:
                if energy != energy_other:
                    grad_energy_mix_other = -invest_copy[GlossaryCore.EnergyInvestmentsValue].values * \
                                            ...
                                            norm_mix.values ** 2
                    self.set_partial_derivative_for_other_types(
                        (f'{energy}.{GlossaryCore.InvestLevelValue}', GlossaryCore.InvestValue), ('invest_energy_mix', energy_other),
        ...
        if 'Invest Distribution' in charts:
            ...
            for energy in energy_list:
                ...
                serie = InstanciatedSeries(
                    energy_invest_df[GlossaryCore.Years].values.tolist(),
            ...
            for year in years_list:
                values = [energy_invest_df.loc[energy_invest_df[GlossaryCore.Years]
