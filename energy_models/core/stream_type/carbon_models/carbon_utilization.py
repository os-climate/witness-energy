...
class CarbonUtilization(BaseStream):
    ...
    food_products_name = 'beverage food'
    fuel_production_name = 'fuel production'
    unit = 'Mt'
    default_techno_list = ['fuel_production.AlgaeCultivation',
                           'food_products.BeverageCarbonation',
                           'food_products.CarbonatedWater',
                           'food_products.FoodProductsTechnoDiscipline',
                           ]
    ...
    #
    #     self.food_products_percentage = None
    #     self.carbon_utilization_type = None
    #     self.food_products_production = None
    #     self.food_products_prod_ratio = None
    #     self.fs_ratio = None
    ...
    #     self.food_products_production = inputs_dict['food_products_production'][self.food_products_name].values
    #     self.food_products_prod_ratio = inputs_dict['food_products_prod_ratio']
    #
    #     self.subelements_list_fgc = []
    #     self.subelements_list_dac = []
    #     for itm in self.subelements_list:
    #         if 'food_products' in itm:
    ...
    #     '''
    #
    #     _, self.consumption_woratio, _, self.carbon_utilization_type_woratio, self.food_products_percentage_woratio, self.fs_ratio_woratio = self.compute_production(
    #         self.sub_production_dict, self.sub_consumption_woratio_dict)
    #
    #     self.production, self.consumption, self.production_by_techno, self.carbon_utilization_type, self.food_products_percentage, self.fs_ratio = self.compute_production(
    ...
    #                                          'food_products': 0.0,
    #                                          'FSA': 0.0,
    #                                          'food_products_limited': 0.0})
    #     food_products_percentage = None
    #     fs_ratio = None
    #
    #     diff_food_products = self.food_products_production - \
    #                    carbon_utilization_type['food_products'].values
    #     if min(diff_food_products.real) < 0:
    #         if carbon_utilization_type['food_products'].values.dtype != self.food_products_production.dtype:
    #             self.food_products_production = self.food_products_production.astype(
    #                 carbon_utilization_type['food_products'].values.dtype)
    #         fs_ratio = np.divide(
    #             self.food_products_production, carbon_utilization_type['food_products'].values, where=carbon_utilization_type['food_products'].values != 0.0, out=np.zeros_like(self.food_products_production))
    #         food_products_percentage = self.compute_food_products_with_exp_min(
    #             fs_ratio)
    #
    #     else:
    #         carbon_utilization_type['food_products limited'] = carbon_utilization_type['food_products']
    #
    #     for element in self.subelements_list_fgc:
    #
    #         if element.startswith('food_products') and min(diff_food_products.real) < 0:
    #             factor = food_products_percentage
    #         else:
    #             factor = 1.0
    ...
    #
    #         if element.startswith('food_products'):
    #             carbon_utilization_type['food_products'] += production_by_techno[
    ...
    #             fs_ratio)
    #         carbon_utilization_type['food_products limited'] = carbon_utilization_type['food_products'] * \
    #             food_products_percentage
    #         production[f'{self.name}'] = carbon_utilization_type['food_products limited'] + \
    ...
    #
    #     return production, consumption, production_by_techno, carbon_utilization_type, food_products_percentage, fs_ratio
    #
    #
    #
    # def compute_food_products_with_exp_min(self, fs_perc):
    #
    #     f = 1.0 - fs_perc
    #     min_prod = 0.001
    ...
    #
    # def compute_dfood_products_with_exp_min(self, fs_ratio):
    #
    #     f = 1.0 - fs_ratio
    #     min_prod = 0.001
    #     dfood_products = np.ones(len(fs_ratio))
    #     if f.min() < min_prod:
    #         f[f < np.log(1e-30) * min_prod] = np.log(1e-30) * min_prod
    #         dfood_products[f < min_prod] = np.exp(
    #             f[f < min_prod] / min_prod) * np.exp(-1) / 10.0
    #     return dfood_products
    #
    ...
    #             if column_df.startswith('food_products') and self.food_products_percentage is not None:
    #                 self.land_use_required[column_df] = element[column_df] * \
    #                     self.food_products_percentage
    #             else:
    #                 self.land_use_required[column_df] = element[column_df]
    #
    ...
    #             production_by_techno, elements_dict, min_prod)
    #
    #     if self.food_products_percentage is not None:
    #         dfoodstorage = self.compute_dfood_products_with_exp_min(
    #             self.fs_ratio)
    #         dfs_ratio = -1.0 * self.food_products_production / \
    #             (self.carbon_utilization_type['food_products'].values)**2
    #         dfs_ratio_dfs_prod = 1.0 / \
    #             self.carbon_utilization_type['food_products'].values
    ...
    #                 ratio_on_old = self.food_products_percentage + prodi * dfoodstorage * dfs_ratio
    #
    ...
    #             # if element_other.startswith('food_products') and self.food_products_percentage is not None:
    ...
    #                     prod_total_for_mix_weight**2
    #
    #                 if element.startswith('food_products') and self.food_products_percentage is not None:
    #
    #                     #(fexpp(fg_ratio) + prod1*f'expp(fg_ratio)*dfg_ratio
    #                     prodi = self.sub_production_dict[element][
    #                         f'{self.name} ({self.unit})'].values
    ...
    #                         if element_bis not in [element, element_other] and element_bis.startswith('food_products'):
    ...
    #                 if element_other.startswith('food_products') and element.startswith('food_products') and self.food_products_percentage is not None:
    #                     # for dpi/dpi
    #                     prodj = self.sub_production_dict[element_other][
    #                         f'{self.name} ({self.unit})'].values
    #                     # dp2 = f'exp(pbis2)*prod2*dfluegas*dfg_ratio
