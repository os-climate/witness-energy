...
from energy_models.models.carbon_utilization.fuel_production.fuel_techno.fuel_techno_disc import FuelProductionTechnoDiscipline
...
class FuelProductionDiscipline(SoSWrapp):
    # ontology information
    _ontology_data = {
        'label': 'Fuel Production Model',
        'type': 'Research',
    ...
    POSSIBLE_fuel_production_TECHNOS = {
                                 'carbon_utilization.fuel_production.AlgaeCultivation': AlgaeCultivationDiscipline.FUEL_PRODUCTION_RATIO,
        'carbon_utilization.fuel_production.FuelProductionApplicationsTechno': FuelProductionTechnoDiscipline.FUEL_PRODUCTION_RATIO
    ...
    DESC_IN = {GlossaryEnergy.YearStart: ClimateEcoDiscipline.YEAR_START_DESC_IN,
               ...
               GlossaryEnergy.techno_list: {'type': 'list', 'subtype_descriptor': {'list': 'string'},
                                     'possible_values': list(POSSIBLE_fuel_production_TECHNOS.keys()),
                                     'visibility': SoSWrapp.SHARED_VISIBILITY, 'namespace': 'ns_fuel_production',
    ...
    energy_name = FuelProduction.name

    DESC_OUT = {GlossaryEnergy.FuelProductionMean: {'type': 'dataframe',
                                  'visibility': SoSWrapp.SHARED_VISIBILITY,
                                  'namespace': 'ns_fuel_production', 'unit': '%'},
                'fuel_production_production': {'type': 'dataframe',
                                        'visibility': SoSWrapp.SHARED_VISIBILITY,
                                        'namespace': 'ns_fuel_production', 'unit': 'Mt'},
                'fuel_production_prod_ratio': {'type': 'dataframe',
                                        'visibility': SoSWrapp.SHARED_VISIBILITY,
                                        'namespace': 'ns_fuel_production', 'unit': '%'}}

    def init_execution(self):
        inputs_dict = self.get_sosdisc_inputs()
        self.energy_model = FuelProduction(self.energy_name)
                ...
                for techno in techno_list:
                    ...
                    dynamic_inputs[f'{techno}.{GlossaryEnergy.TechnoProductionValue}'] = {
                        ...
                        'dataframe_descriptor': {GlossaryEnergy.Years: ('int', [1900, GlossaryEnergy.YeartEndDefault], False),
                                                 'fuel production (TWh)': ('float', None, False),
                                                 }
                    }
                    ...
                    dynamic_inputs[f'{techno}.fuel_production_co2_ratio'] = {'type': 'array',
                                                                      'visibility': SoSWrapp.SHARED_VISIBILITY,
                                                                      'namespace': ns_variable, 'unit': '',
                                                                      'default': self.POSSIBLE_fuel_production_TECHNOS[techno]}
    ...
    def run(self):
        ...
        fuel_production_mean = self.energy_model.compute(inputs_dict)

        outputs_dict = {
            GlossaryEnergy.FuelProductionMean: fuel_production_mean,
            'fuel_production_production': self.energy_model.get_total_fuel_production_production(),
            'fuel_production_prod_ratio': self.energy_model.get_total_fuel_production_prod_ratio()}
        # -- store outputs
    ...
    def compute_sos_jacobian(self):
        ...
        mix_weights = self.get_sosdisc_outputs('fuel_production_prod_ratio')

        total_prod = self.get_sosdisc_outputs('fuel_production_production')[
            self.energy_model.name].values
        len_matrix = len(total_prod)
        for techno in technologies_list:

            self.set_partial_derivative_for_other_types(
                (GlossaryEnergy.FuelProductionMean,
                 GlossaryEnergy.FuelProductionMean), (f'{techno}.fuel_production_co2_ratio',),
                np.reshape(mix_weights[techno].values, (len_matrix, 1)))

            # self.set_partial_derivative_for_other_types(
            #     (GlossaryEnergy.FlueGasMean,
            #      GlossaryEnergy.FlueGasMean), (f'{techno}.fuel_production_co2_ratio',),
            #     np.reshape(mix_weights1[techno].values, (len_matrix, 1)))

            # An array of one value because GEMS needs array
            fuel_production_co2_ratio = self.get_sosdisc_inputs(
                f'{techno}.fuel_production_co2_ratio')[0]
            ...
            self.set_partial_derivative_for_other_types(
                ('fuel_production_prod_ratio', techno),
            ...
            grad_FuelProduction_prod = fuel_production_co2_ratio * grad_prod
            for techno_other in technologies_list:
                if techno != techno_other:
                    fuel_production_co2_ratio_other = self.get_sosdisc_inputs(
                        f'{techno_other}.fuel_production_co2_ratio')[0]

                    grad_fuel_production_prod_ratio = -self.energy_model.production[
                    ...
                    self.set_partial_derivative_for_other_types(
                        ...
                        inputs_dict['scaling_factor_techno_production'] * np.identity(
                            len_matrix) * grad_fuel_production_prod_ratio)

                    grad_FuelProduction_prod -= \
                        fuel_production_co2_ratio_other * self.energy_model.production[
                            f'{self.energy_model.name} {techno_other} (Mt)'].values / \
                        total_prod ** 2

            self.set_partial_derivative_for_other_types(
                (GlossaryEnergy.FuelProductionMean, GlossaryEnergy.FuelProductionMean),
                (f'{techno}.{GlossaryEnergy.TechnoProductionValue}',
                 f'{self.energy_model.name} (Mt)'),
                inputs_dict['scaling_factor_techno_production'] * np.identity(len_matrix) * grad_FuelProduction_prod)

            self.set_partial_derivative_for_other_types(
                ('fuel_production_production', self.energy_model.name),
                (f'{techno}.{GlossaryEnergy.TechnoProductionValue}',
                 f'{self.energy_model.name} (Mt)'),
    ...
    def get_post_processing_list(self, filters=None):
        ...
        if 'Food storage production' in charts:
            new_chart = self.get_fuel_production_production()
    ...
    def get_fuel_production_production(self):
        fuel_production_total = self.get_sosdisc_outputs(
            'fuel_production_production')[self.energy_name].values
        fuel_production_prod_ratio = self.get_sosdisc_outputs('fuel_production_prod_ratio')
        technologies_list = self.get_sosdisc_inputs(GlossaryEnergy.techno_list)
        years = fuel_production_prod_ratio[GlossaryEnergy.Years].values
        ...
        for techno in technologies_list:
            fuel_production_prod = fuel_production_total * fuel_production_prod_ratio[techno].values
            serie = InstanciatedSeries(
                years.tolist(),
                fuel_production_prod.tolist(), techno.split('.')[-1], 'bar')
            new_chart.series.append(serie)

        serie = InstanciatedSeries(
            years.tolist(),
            fuel_production_total.tolist(), 'Total', 'lines')
        new_chart.series.append(serie)

        return new_chart

    def get_chart_average_co2_concentration(self):
        fuel_production_co2_concentration = self.get_sosdisc_outputs(GlossaryEnergy.FuelProductionMean)
        ...
        serie = InstanciatedSeries(
            fuel_production_co2_concentration[GlossaryEnergy.Years].values.tolist(),
            (fuel_production_co2_concentration[GlossaryEnergy.FuelProductionMean].values * 100).tolist(), f'CO2 concentration', 'lines')
    ...
    def get_table_technology_co2_concentration(self):
        ...
        for techno in technologies_list:
            val_co2 = round(self.get_sosdisc_inputs(
                f'{techno}.fuel_production_co2_ratio')[0] * 100, 2)
