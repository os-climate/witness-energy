...
class DirectAirDiscipline(SoSWrapp):
    ...
    POSSIBLE_FLUE_GAS_TECHNOS = {
                                 # f'{GlossaryEnergy.electricity}.CoalGen': CoalGenDiscipline.FLUE_GAS_RATIO,
    ...
    def init_execution(self):
        ...
        self.energy_model = DirectAir(self.energy_name)
    ...
    def run(self):
        ...
        outputs_dict = {
            GlossaryEnergy.DirectAirMean: flue_gas_mean,
            'direct_air_production': self.energy_model.get_total_direct_air_production(),
            'direct_air_prod_ratio': self.energy_model.get_total_direct_air_prod_ratio()}
        # -- store outputs
    ...
    #     mix_weights = self.get_sosdisc_outputs('direct_air_prod_ratio')
    #
    #     total_prod = self.get_sosdisc_outputs('direct_air_production')[
    #         self.energy_model.name].values
    #     len_matrix = len(total_prod)
    #     for techno in technologies_list:
    #
    #         self.set_partial_derivative_for_other_types(
    #             (GlossaryEnergy.DirectAirMean,
    #              GlossaryEnergy.DirectAirMean), (f'{techno}.flue_gas_co2_ratio',),
    ...
    #
    #                 grad_direct_air_prod_ratio = -self.energy_model.production[
    ...
    #                         len_matrix) * grad_direct_air_prod_ratio)
    #
    #                 grad_fluegas_prod -= \
    ...
    #
    #         self.set_partial_derivative_for_other_types(
    #             (GlossaryEnergy.DirectAirMean, GlossaryEnergy.DirectAirMean),
    #             (f'{techno}.{GlossaryEnergy.TechnoProductionValue}',
    #              f'{self.energy_model.name} (Mt)'),
    #             inputs_dict['scaling_factor_techno_production'] * np.identity(len_matrix) * grad_fluegas_prod)
    #
    #         self.set_partial_derivative_for_other_types(
    #             ('direct_air_production', self.energy_model.name),
    #             (f'{techno}.{GlossaryEnergy.TechnoProductionValue}',
    #              f'{self.energy_model.name} (Mt)'),
    ...
    def get_post_processing_list(self, filters=None):
        ...
        if 'Flue gas production' in charts:
            new_chart = self.get_direct_air_production()
    ...
    def get_direct_air_production(self):
        flue_gas_total = self.get_sosdisc_outputs(
            'direct_air_production')[self.energy_name].values
        direct_air_prod_ratio = self.get_sosdisc_outputs('direct_air_prod_ratio')
        technologies_list = self.get_sosdisc_inputs(GlossaryEnergy.techno_list)
        years = direct_air_prod_ratio[GlossaryEnergy.Years].values
        ...
        for techno in technologies_list:
            flue_gas_prod = flue_gas_total * direct_air_prod_ratio[techno].values
    ...
    def get_chart_average_co2_concentration(self):
        flue_gas_co2_concentration = self.get_sosdisc_outputs(GlossaryEnergy.DirectAirMean)
        ...
        serie = InstanciatedSeries(
            ...
            (flue_gas_co2_concentration[GlossaryEnergy.DirectAirMean].values * 100).tolist(), f'CO2 concentration', 'lines')
