...
class Energy_Mix_Discipline(SoSWrapp):
    ...
    heat_losses_percentage_default = (
        (heat_use_energy_2019 - heat_tfc_2019) / total_raw_prod_2019 * 100
    )

    loss_percentage_default_dict = {energy: 0.0 for energy in EnergyMix.energy_list}
    loss_percentage_default_dict[GlossaryEnergy.methane] = (
        289.21 / FossilGasDiscipline.initial_production * 100.0
    )
    ...
    ] = 90.15 / RefineryDiscipline.initial_production * 100.0
    ...
    DESC_IN = {
        ...
        "solid_fuel_elec_constraint_ref": {
            "type": "float",
            "default": 10000.0,
            "unit": "Twh",
            "user_level": 2,
        ...
        "liquid_hydrogen_constraint_ref": {
            "type": "float",
            "default": 1000.0,
            "unit": "Twh",
            "user_level": 2,
        },
        "ref_constraint_non_use_capital_energy": {
            "type": "float",
            "default": 0.30,
            "unit": "-",
            "description": "0.30 means after 35 % of capital not used the constraint will explode",
        },
        "tol_constraint_non_use_capital_energy": {
            "type": "float",
            "default": 0.05,
            "unit": "-",
            "description": "0.05 means constraint does not penalize lagrangian when non use capital is less than 5%",
        },
        "period_tol_power_non_use_capital_constraint": {
            "type": "float",
            "default": 1.0,
            "unit": "-",
            "description": "0.05 means constraint does not penalize lagrangian when non use capital is less than 5%",
        ...
        "syngas_prod_constraint_limit": {
            "type": "float",
            "default": 10000.0,
            "unit": "TWh",
            "user_level": 2,
        },
        "ratio_ref": {"type": "float", "default": 500.0, "unit": "-", "user_level": 2},
        "heat_losses_percentage": {
            "type": "float",
            "default": heat_losses_percentage_default,
            "unit": "%",
            "range": [0.0, 100.0],
        },
    }
    ...
    DESC_OUT = {
        ...
        GlossaryEnergy.ConstraintEnergyNonUseCapital: {
            "type": "dataframe",
            "unit": "-",
    ...
    def setup_sos_disciplines(self):
        ...
        if GlossaryEnergy.energy_list in self.get_data_in():
            ...
            if energy_list is not None:
                for energy in energy_list:
                    ...
                    dynamic_inputs[
                        f"{ns_energy}.{GlossaryEnergy.EnergyTypeCapitalDfValue}"
                    ] = GlossaryEnergy.get_dynamic_variable(
                        GlossaryEnergy.EnergyTypeCapitalDf
                    )
                    ...
                    if energy in self.energy_class_dict:
                        # Biomass energy is computed by the agriculture model
                        dynamic_inputs[
                            f"{ns_energy}.{GlossaryEnergy.CO2EmissionsValue}"
                        ] = {
                            "type": "dataframe",
                            "unit": "kg/kWh",
                            "dynamic_dataframe_columns": True,
                        ...
                        dynamic_inputs[f"{ns_energy}.losses_percentage"] = {
                            "type": "float",
                            "unit": "%",
                            "default": self.loss_percentage_default_dict[energy],
                            "range": [0.0, 100.0],
                            "dataframe_descriptor": {
                                GlossaryEnergy.Years: ("float", None, True),
            ...
            if GlossaryEnergy.ccs_list in self.get_data_in():
                ...
                if ccs_list is not None:
                    for ccs_name in ccs_list:
                        etcp = GlossaryEnergy.get_dynamic_variable(
                            GlossaryEnergy.EnergyTypeCapitalDf
                        )
                        etcp.update({"namespace": "ns_ccs", "visibility": "Shared"})
                        dynamic_inputs[
                            f"{ccs_name}.{GlossaryEnergy.EnergyTypeCapitalDfValue}"
                        ] = etcp

                        dynamic_inputs[
                            f"{ccs_name}.{GlossaryEnergy.StreamConsumptionValue}"
                        ] = {
                            "type": "dataframe",
                            "unit": "PWh",
                            "visibility": SoSWrapp.SHARED_VISIBILITY,
                        ...
                        dynamic_inputs[
                            f"{ccs_name}.{GlossaryEnergy.StreamConsumptionWithoutRatioValue}"
                        ] = {
                            "type": "dataframe",
                            "unit": "PWh",
                            "visibility": SoSWrapp.SHARED_VISIBILITY,
                        ...
                        dynamic_inputs[
                            f"{ccs_name}.{GlossaryEnergy.EnergyProductionValue}"
                        ] = {
                            "type": "dataframe",
                            "unit": "PWh",
                            "visibility": SoSWrapp.SHARED_VISIBILITY,
                        ...
                        dynamic_inputs[
                            f"{ccs_name}.{GlossaryEnergy.LandUseRequiredValue}"
                        ] = {
                            "type": "dataframe",
                            "unit": "Gha",
                            "visibility": SoSWrapp.SHARED_VISIBILITY,
        ...
        if (
            GlossaryEnergy.energy_list in self.get_data_in()
            and GlossaryEnergy.ccs_list in self.get_data_in()
        ):
            ...
            if energy_list is not None and ccs_list is not None:
                all_stream_variable = GlossaryEnergy.get_dynamic_variable(
                    GlossaryEnergy.AllStreamsDemandRatio
                )
                df_descr = {
                    energy_name: ("float", None, True)
                    for energy_name in energy_list + ccs_list
                }
                all_stream_variable["dataframe_descriptor"].update(df_descr)
                dynamic_outputs[GlossaryEnergy.AllStreamsDemandRatioValue] = (
                    all_stream_variable
                )
    ...
    def run(self):
        ...
        energy_production_objective = np.asarray(
            [
                (1.0 - alpha)
                * self.energy_model.production[GlossaryEnergy.TotalProductionValue][0]
                * delta_years
                / self.energy_model.production[
        ...
        scaled_energy_production = pd.DataFrame(
            {
                GlossaryEnergy.Years: self.energy_model.production[
                    GlossaryEnergy.Years
                ].values,
                GlossaryEnergy.TotalProductionValue: self.energy_model.production[
                    GlossaryEnergy.TotalProductionValue
                ].values
                / inputs_dict["scaling_factor_energy_production"],
            }
        )

        outputs_dict = {
            GlossaryEnergy.StreamPricesValue: self.energy_model.stream_prices,
        ...
        if (
            f"production {self.LIQUID_FUEL_NAME} ({GlossaryEnergy.energy_unit})"
            in self.energy_model.production
            and f"production {self.HYDROGEN_NAME} ({GlossaryEnergy.energy_unit})"
            in self.energy_model.production
            and f"production {GlossaryEnergy.hydrogen}.{GlossaryEnergy.liquid_hydrogen} ({GlossaryEnergy.energy_unit})"
            in self.energy_model.production
        ):
            ...
            production_hydrogen = self.energy_model.production[
                f"production {self.HYDROGEN_NAME} ({GlossaryEnergy.energy_unit})"
            ]
            ...
            sum_energies_production = (
                production_liquid_fuel
                + production_hydrogen
                + production_liquid_hydrogen
            )

            energies_production_constraint = (
                sum_energies_production
                - primary_energy_percentage
                * self.energy_model.production[GlossaryEnergy.TotalProductionValue]
            )

            outputs_dict["primary_energies_production"] = (
                energies_production_constraint.to_frame("primary_energies")
            )
        else:
    ...
    def update_biomass_dry_name(self, inputs_dict_orig, inputs_dict):
        ...
        if GlossaryEnergy.biomass_dry in energy_list:
            inputs_dict[
                f"{BiomassDry.name}.{GlossaryEnergy.StreamConsumptionValue}"
            ] = inputs_dict_orig.pop(
                f"{agri_name}.{GlossaryEnergy.StreamConsumptionValue}"
            )
            inputs_dict[
                f"{BiomassDry.name}.{GlossaryEnergy.StreamConsumptionWithoutRatioValue}"
            ] = inputs_dict_orig.pop(
                f"{agri_name}.{GlossaryEnergy.StreamConsumptionWithoutRatioValue}"
            )
            inputs_dict[f"{BiomassDry.name}.{GlossaryEnergy.EnergyProductionValue}"] = (
                inputs_dict_orig.pop(
                    f"{agri_name}.{GlossaryEnergy.EnergyProductionValue}"
                )
            )
            inputs_dict[f"{BiomassDry.name}.{GlossaryEnergy.StreamPricesValue}"] = (
                inputs_dict_orig.pop(f"{agri_name}.{GlossaryEnergy.StreamPricesValue}")
            )
            inputs_dict[f"{BiomassDry.name}.{GlossaryEnergy.LandUseRequiredValue}"] = (
                inputs_dict_orig.pop(
                    f"{agri_name}.{GlossaryEnergy.LandUseRequiredValue}"
                )
            )
            inputs_dict[f"{BiomassDry.name}.{GlossaryEnergy.CO2EmissionsValue}"] = (
                inputs_dict_orig.pop(f"{agri_name}.{GlossaryEnergy.CO2EmissionsValue}")
            )
            inputs_dict[f"{BiomassDry.name}.{GlossaryEnergy.CO2PerUse}"] = (
                inputs_dict_orig.pop(f"{agri_name}.{GlossaryEnergy.CO2PerUse}")
            )
    ...
    def compute_sos_jacobian(self):
        ...
        inputs_dict.update(inputs_dict_orig)
        stream_list = (
            inputs_dict[GlossaryEnergy.energy_list]
        ...
        primary_energy_percentage = inputs_dict["primary_energy_percentage"]
        production_detailed_df = outputs_dict[
        ...
        energies = [
            ...
            if j not in [GlossaryEnergy.carbon_storage, GlossaryEnergy.carbon_capture]
        ]
        ...
        for stream in stream_list:
            sub_production_dict[stream] = (
                inputs_dict[f"{stream}.{GlossaryEnergy.EnergyProductionValue}"]
                * scaling_factor_energy_production
            )
            sub_consumption_dict[stream] = (
                inputs_dict[f"{stream}.{GlossaryEnergy.StreamConsumptionValue}"]
                * scaling_factor_energy_consumption
            )
        ...
        # -------------------------------------------#
        d_non_use_capital, d_capital = (
            self.energy_model.d_non_use_capital_constraint_d_capital()
        )
        d_non_use_capital_obj, d_capital_obj = (
        ...
        for stream in (
            ...
            d_non_use_capital_by_stream, d_capital_by_stream = (
                self.energy_model.d_non_use_capital_obj_by_stream_d_capital(ns_stream)
            )
            self.set_partial_derivative_for_other_types(
                (GlossaryEnergy.ObjectiveEnergyNonUseCapitalByStream,),
                (
                    f"{ns_stream}.{GlossaryEnergy.EnergyTypeCapitalDfValue}",
        ...
        for stream in stream_list:
            ...
            if stream in energies:
                ...
                if stream in self.energy_model.raw_tonet_dict:
                    loss_percentage += 1.0 - self.energy_model.raw_tonet_dict[stream]
                ...
                dprod_objective_dprod = self.compute_denergy_production_objective_dprod(
                    dtotal_prod_denergy_prod,
                    inputs_dict["alpha"],
                    outputs_dict[GlossaryEnergy.EnergyProductionValue],
                    years,
                )
                self.set_partial_derivative_for_other_types(
                    (
                ...
                target_production_constraint_ref = inputs_dict[
                    GlossaryEnergy.TargetProductionConstraintRefValue
                ]
                self.set_partial_derivative_for_other_types(
                    (
                ...
                self.set_partial_derivative_for_other_types(
                    ("energy_production_brut", GlossaryEnergy.TotalProductionValue),
                    (f"{ns_stream}.{GlossaryEnergy.EnergyProductionValue}", stream),
                ...
                if (
                    f"production {self.LIQUID_FUEL_NAME} ({GlossaryEnergy.energy_unit})"
                    in production_detailed_df.columns
                    and f"production {self.HYDROGEN_NAME} ({GlossaryEnergy.energy_unit})"
                    in production_detailed_df.columns
                    and f"production {GlossaryEnergy.hydrogen}.{GlossaryEnergy.liquid_hydrogen} ({GlossaryEnergy.energy_unit})"
                    in production_detailed_df.columns
                    ...
                    if stream in [
                        ...
                        self.set_partial_derivative_for_other_types(
                            ...
                            (
                                np.identity(len(years)) * (1 - loss_percentage)
                                - primary_energy_percentage * dtotal_prod_denergy_prod
                            )
                            * scaling_factor_energy_production,
                        )
                    else:
                ...
                if stream in self.energy_model.energy_constraint_list:
                    self.set_partial_derivative_for_other_types(
                        (
                            f"{EnergyMix.CONSTRAINT_PROD_SOLID_FUEL_ELEC}",
                            "constraint_solid_fuel_elec",
                        ),
                        (f"{ns_stream}.{GlossaryEnergy.EnergyProductionValue}", stream),
                        (
                            -scaling_factor_energy_production
                            * (
                                (1 - loss_percentage)
                                - solid_fuel_elec_percentage * dtotal_prod_denergy_prod
                            )
                            / solid_fuel_elec_constraint_ref
                        )
                        * np.identity(len(years)),
                ...
                if (
                    ...
                    self.set_partial_derivative_for_other_types(
                        (
                            f"{EnergyMix.CONSTRAINT_PROD_H2_LIQUID}",
                            "constraint_liquid_hydrogen",
                        ),
                        (f"{ns_stream}.{GlossaryEnergy.EnergyProductionValue}", stream),
                        (
                            -scaling_factor_energy_production
                            * (liquid_hydrogen_percentage - 1)
                            / liquid_hydrogen_constraint_ref
                        )
                        * np.identity(len(years)),
                    )
                ...
                for stream_input in stream_list:
                    ...
                    list_columns_energy_consumption = list(
                        inputs_dict[
                            f"{stream_input}.{GlossaryEnergy.StreamConsumptionValue}"
                        ].columns
                    )
                    if (
                        f"{stream} ({GlossaryEnergy.unit_dicts[stream]})"
                        in list_columns_energy_consumption
                    ):
                        ...
                        dprod_objective_dcons = (
                            self.compute_denergy_production_objective_dprod(
                                dtotal_prod_denergy_cons,
                                inputs_dict["alpha"],
                                outputs_dict[GlossaryEnergy.EnergyProductionValue],
                                years,
                        ...
                        self.set_partial_derivative_for_other_types(
                            ...
                            * dtotal_prod_denergy_cons
                            / scaling_factor_energy_production
                            * 1e3
                            / target_production_constraint_ref,
                        )
                        self.set_partial_derivative_for_other_types(
                            (
                                GlossaryEnergy.StreamProductionDetailedValue,
                ...
                for stream_input in stream_list:
                    ...
                    if (
                        f"{stream} ({GlossaryEnergy.unit_dicts[stream]})"
                        in list_columns_energy_consumption
                    ):  # F or stream_input == GlossaryEnergy.carbon_storage:
                        self.set_partial_derivative_for_other_types(
                            (
                                GlossaryEnergy.StreamProductionDetailedValue,
                                f"production {stream} ({GlossaryEnergy.unit_dicts[stream]})",
                    ...
                    if (
                        f"{stream} ({GlossaryEnergy.unit_dicts[stream]})"
                        in list_columns_energy_consumption
                    ):  # or stream_input == GlossaryEnergy.carbon_capture:
                        self.set_partial_derivative_for_other_types(
                            (
                                GlossaryEnergy.StreamProductionDetailedValue,
                                f"production {stream} ({GlossaryEnergy.unit_dicts[stream]})",
        ...
        for stream in stream_list:
            ...
            if stream in energies:
                ...
                self.set_partial_derivative_for_other_types(
                    ...
                    inputs_dict[f"{stream}.{GlossaryEnergy.CO2PerUse}"][
                ...
                self.set_partial_derivative_for_other_types(
                    ("energy_prices_after_tax", stream),
                    (
                        f"{ns_stream}.{GlossaryEnergy.CO2PerUse}",
                        GlossaryEnergy.CO2PerUse,
                    ),
                    inputs_dict[GlossaryEnergy.CO2TaxesValue][
        ...
        for stream in stream_list:
            ns_stream = self.get_ns_stream(stream)
            for resource in inputs_dict[
                f"{stream}.{GlossaryEnergy.StreamConsumptionValue}"
            ]:
                ...
                if resource_wo_unit in resource_list:
                    self.set_partial_derivative_for_other_types(
                        ...
                        (
                            f"{ns_stream}.{GlossaryEnergy.StreamConsumptionValue}",
                            resource,
                        ),
                    ...
                    self.set_partial_derivative_for_other_types(
                        ...
                        (
                            f"{ns_stream}.{GlossaryEnergy.StreamConsumptionWithoutRatioValue}",
                            resource,
                        ),
                        scaling_factor_energy_consumption * np.identity(len(years)),
                    )
        ...
        self.grad_energy_mix_vs_prod_dict = (
            self.energy_model.compute_grad_element_mix_vs_prod(
                deepcopy(production_energy_net_pos),
                element_dict,
            ...
            if stream in energies:
                ...
                self.set_partial_derivative_for_other_types(
                    (
                        ...
                        GlossaryEnergy.EnergyPriceValue,
                    ),
                    (f"{ns_stream}.{GlossaryEnergy.StreamPricesValue}", stream),
                    d_emp,
                )
                d_emp_obj = (
                    self.energy_model.d_energy_mean_price_obj_d_energy_mean_price(d_emp)
                )
                self.set_partial_derivative_for_other_types(
            ...
            for stream_input in stream_list:
                if stream_input in energies:
                    ...
                    if (
                        ...
                        if stream in energies:
                            dmean_price_dcons = self.compute_dmean_price_dprod(
                                stream,
                                energies,
                                mix_weight,
                                energy_price_after_tax,
                            ...
                            d_emp_obj = self.energy_model.d_energy_mean_price_obj_d_energy_mean_price(
                                d_emp
                            )
                            self.set_partial_derivative_for_other_types(
                                (GlossaryEnergy.EnergyMeanPriceObjectiveValue,),
        ...
        co2_emissions_needed_by_energy_mix = outputs_dict[
            "co2_emissions_needed_by_energy_mix"
        ]
        ...
        for key, value in dtot_co2_emissions.items():
            ...
            if stream in stream_list:
                self.set_gradient_for_co2_emissions(
                    "co2_emissions_needed_by_energy_mix",
                    co2_emissions_needed_by_energy_mix,
                    co2_emission_column,
                    stream,
        ...
        for stream in energies:
            ...
            if stream in outputs_dict[GlossaryEnergy.StreamsCO2EmissionsValue].keys():
                self.set_partial_derivative_for_other_types(
                    ...
                    (f"{ns_stream}.{GlossaryEnergy.CO2EmissionsValue}", stream),
                    np.identity(len(years)),
                )
            for stream_input in stream_list:
                ns_stream_input = self.get_ns_stream(stream_input)
                list_columnsenergyprod = list(
                    inputs_dict[
                        f"{stream_input}.{GlossaryEnergy.EnergyProductionValue}"
                    ].columns
                )
                list_columns_energy_consumption = list(
                    inputs_dict[
                ...
                list_index_conso = [
                    j == f"{stream} ({GlossaryEnergy.unit_dicts[stream]})"
                    for j in list_columns_energy_consumption
                ]

                if True in list_index_prod:
                    loss_percentage = (
                        inputs_dict[f"{ns_stream}.losses_percentage"] / 100.0
                    )
                    # To model raw to net percentage for witness coarse
                    # energies
                    if stream in self.energy_model.raw_tonet_dict:
                        loss_percentage += (
                            1.0 - self.energy_model.raw_tonet_dict[stream]
                        )
                    loss_percent = heat_losses_percentage + loss_percentage
        ...
        # --------------------------------------#
        all_streams_demand_ratio = outputs_dict[
            GlossaryEnergy.AllStreamsDemandRatioValue
        ]
        energy_production_brut_detailed = outputs_dict[
        ...
        for stream in stream_list:
            ns_stream = self.get_ns_stream(stream)
            ddemand_ratio_denergy_prod, ddemand_ratio_denergy_cons = (
                self.compute_ddemand_ratio_denergy_production(
                    stream,
                    sub_production_dict,
                    sub_consumption_woratio_dict,
                    scaling_factor_energy_production,
                    years,
                    energy_production_brut_detailed,
                )
            ...
            dobjective_dratio_energy = np.array(
                [
                    dobjective_dratio[ienergy + iyear * len(stream_list)]
            ...
            for stream_input in stream_list:
                ...
                if (
                    ...
                    self.set_partial_derivative_for_other_types(
                        (GlossaryEnergy.AllStreamsDemandRatioValue, f"{stream}"),
                        (
                            f"{ns_stream_input}.{GlossaryEnergy.StreamConsumptionWithoutRatioValue}",
                            f"{stream} ({GlossaryEnergy.unit_dicts[stream]})",
                        ),
        ...
        for stream in stream_list:
            ...
            for key in outputs_dict["land_demand_df"]:
                if (
                    key
                    in inputs_dict[f"{stream}.{GlossaryEnergy.LandUseRequiredValue}"]
                    and key != GlossaryEnergy.Years
                ):
                    self.set_partial_derivative_for_other_types(
                        ("land_demand_df", key),
    ...
    def set_gradient_for_co2_emissions(
        self,
        co2_variable,
        co2_emissions,
        co2_emission_column,
            ...
            if last_part_key == "prod":
                ...
                for energy_df in inputs_dict[GlossaryEnergy.energy_list]:
                    ...
                    list_columnsenergycons = list(
                        inputs_dict[
                            f"{energy_df}.{GlossaryEnergy.StreamConsumptionValue}"
                        ].columns
                    )
                    if (
                        f"{energy} ({GlossaryEnergy.energy_unit})"
                        in list_columnsenergycons
                    ):
                        self.set_partial_derivative_for_other_types(
                            (co2_variable, co2_emission_column),
                            (
    ...
    def compute_denergy_production_objective_dprod(
        self, dtotal_production_denergy_production, alpha, prod, years
    ):
        ...
        dtot_energy_production_sum = dtotal_production_denergy_production.sum(axis=0)
        ...
        u = (1.0 - alpha) * tot_energy_production_0 * delta_years
        v = tot_energy_production_sum
        u_prime = (1.0 - alpha) * dtot_energy_production_0 * delta_years
        v_prime = dtot_energy_production_sum
        dprod_objective_dprod = u_prime / v - u * v_prime / v**2

        return dprod_objective_dprod

    def compute_dproduction_net_denergy_production(
        self, energy, production_df, energy_price
    ):
        """
        ...
        denergy_mean_prod = (
            energy_price[energy]
            * 1e6
            * production_df[GlossaryEnergy.TotalProductionValue]
            - production_df["energy_price_pond"]
        ) / (1e6 * (production_df[GlossaryEnergy.TotalProductionValue]) ** 2)

        # derivative of negative prod is 0
        index_l = production_df[
            production_df[f"production {energy} ({GlossaryEnergy.energy_unit})"] == 0
        ].index
        denergy_mean_prod.loc[index_l] = 0
        return denergy_mean_prod

    def compute_dtotal_production_denergy_production(
        self, production_detailed_df, min_energy, total_loss_percent
    ):
        """
        ...
        dtotal_production_denergy_production *= 1.0 - total_loss_percent
        ...
        if total_prod.min() < min_energy:
            ...
            dtotal_production_denergy_production[total_prod < min_energy] = (
                np.exp(total_prod[total_prod < min_energy] / min_energy)
                * np.exp(-1)
                / 10.0
    ...
    def compute_ddemand_ratio_denergy_production(
        self,
        energy,
        sub_production_dict,
        sub_consumption_dict,
        scaling_factor_production,
        years,
        energy_production_brut_detailed,
    ):
        ...
        if energy in EnergyMix.raw_tonet_dict.keys():
            ...
            prod_raw_to_substract = energy_production_brut_detailed[
                column_name
            ].values * (1.0 - EnergyMix.raw_tonet_dict[energy])
            ...
            if f"{energy} ({self.stream_class_dict[energy].unit})" in consu.columns:
                energy_consumption = np.sum(
                    [
                        energy_consumption,
                        consu[
                            f"{energy} ({self.stream_class_dict[energy].unit})"
                        ].values,
                    ],
                    axis=0,
                )
        ...
        # corresponding value
        denergy_prod_limited = compute_dfunc_with_exp_min(energy_production, 1.0e-10)
        energy_prod_limited = compute_func_with_exp_min(energy_production, 1.0e-10)

        # If prod < cons, set the identity element for the given year to
        # the corresponding value
        denergy_cons_limited = compute_dfunc_with_exp_min(energy_consumption, 1.0e-10)
        energy_cons_limited = compute_func_with_exp_min(energy_consumption, 1.0e-10)
        ddemand_ratio_denergy_cons = (
            np.identity(len(years))
            * 100.0
            * np.where(
                (energy_prod_limited <= energy_cons_limited)
                * (energy_prod_limited / energy_cons_limited > 1e-15),
                -scaling_factor_production
                * energy_prod_limited
                * denergy_cons_limited
            ...
            denergy_prod_limited = (
                ...
                * np.where(
                    ...
                    * (energy_prod_limited / energy_cons_limited > 1e-15),
                    denergy_prod_limited
                    * scaling_factor_production
                    * EnergyMix.raw_tonet_dict[energy]
                    / energy_cons_limited,
                    0.0,
                )
            )
    ...
    def compute_dmean_price_dprod(
        self,
        energy,
        energies,
        mix_weight,
        energy_price_after_tax,
        production_energy_net_pos_consumable,
        production_detailed_df,
        cons=False,
    ):
        """
        ...
        # with our needs
        grad_energy_mix_vs_prod = self.grad_energy_mix_vs_prod_dict[energy] * np.sign(
            mix_weight_energy
        )
        grad_price_vs_prod = (
            energy_price_after_tax[energy].values * grad_energy_mix_vs_prod
        )
        for energy_other in energies:
            if energy_other != energy:
                ...
                grad_energy_mix_vs_prod = self.grad_energy_mix_vs_prod_dict[
                    f"{energy} {energy_other}"
                ] * np.sign(mix_weight_techno_other)
                grad_price_vs_prod += (
                    energy_price_after_tax[energy_other].values
        ...
        gradient_sign = np.sign(production_energy_net_pos_consumable[energy].values) + (
            production_detailed_df[
                f"production {energy} ({GlossaryEnergy.energy_unit})"
            ].values
            == 0.0
        )
        years = production_detailed_df[GlossaryEnergy.Years].values

        dmean_price_dprod = grad_price_vs_prod * gradient_sign * np.identity(len(years))
        # if dmean_price_dcons
        if cons:
            dmean_price_dprod = (
                -grad_price_vs_prod
                * np.sign(production_energy_net_pos_consumable[energy].values)
                * np.identity(len(years))
            )
    ...
    def get_chart_filter_list(self):
        ...
        chart_list = [
            ...
            "CO2 emissions",
            "Carbon intensity",
            "CO2 taxes over the years",
            "Solid energy and electricity production constraint",
            "Liquid hydrogen production constraint",
            "Stream ratio",
            "Energy mix losses",
    ...
    def get_post_processing_list(self, filters=None):
        ...
        if "Target energy production constraint" in charts:
            ...
            target_energy_production = target_energy_production_df[
                GlossaryEnergy.TargetEnergyProductionValue
            ].values
            ...
            if target_energy_production.max() > 0:
                ...
                serie_production = InstanciatedSeries(
                    list(years), list(energy_production), "Energy production", "bar"
                )
        ...
        if "Energy price" in charts and "$/MWh" in price_unit_list:
            new_chart = (
                self.get_chart_energy_price_in_dollar_kwh_without_production_taxes()
            )
        ...
        if (
            "Solid energy and electricity production constraint" in charts
            and len(list(set(self.energy_constraint_list).intersection(energy_list)))
            > 0
            and f"{GlossaryEnergy.fuel}.{GlossaryEnergy.solid_fuel}" in energy_list
        ):
        ...
        if (
            "Liquid hydrogen production constraint" in charts
            and f"{GlossaryEnergy.hydrogen}.{GlossaryEnergy.liquid_hydrogen}"
            in energy_list
        ):
    ...
    def get_chart_solid_energy_elec_constraint(self):
        ...
        new_serie = InstanciatedSeries(
            list(energy_production_detailed[GlossaryEnergy.Years].values),
            list(sum_solid_fuel_elec),
            "Sum of solid fuel and electricity productions",
            "lines",
        )
        ...
        new_serie = InstanciatedSeries(
            list(energy_production_detailed[GlossaryEnergy.Years].values),
            list(
                energy_production_detailed[GlossaryEnergy.TotalProductionValue].values
                * solid_fuel_elec_percentage
            ),
            f"{100 * solid_fuel_elec_percentage}% of total production",
            "lines",
        )
    ...
    def get_chart_liquid_hydrogen_constraint(self):
        ...
        new_serie = InstanciatedSeries(
            list(energy_production_detailed[GlossaryEnergy.Years].values),
            list(constraint),
            "percentage of total hydrogen production",
            "lines",
        )
        new_chart.series.append(new_serie)
        return new_chart
    ...
    def get_chart_comparison_carbon_intensity(self):
        ...
        for energy in energy_list:
            if self.stream_class_dict[energy].unit == "TWh":
                ...
                serie = InstanciatedSeries(year_list, emission_list, energy, "lines")
    ...
    def get_chart_energy_price_in_dollar_kwh(self):
        ...
        for energy in energy_list:
            if self.stream_class_dict[energy].unit == "TWh":
                max_value = max(max(energy_prices[energy].values.tolist()), max_value)

        new_chart = TwoAxesInstanciatedChart(
            GlossaryEnergy.Years,
            "Prices [$/MWh]",
            primary_ordinate_axis_range=[0, max_value],
        ...
        for energy in energy_list:
            ...
            if self.stream_class_dict[energy].unit == "TWh":
                ...
                serie = InstanciatedSeries(
                    ...
                    techno_price[energy].values.tolist(),
                    energy,
                    "lines",
                )
                new_chart.series.append(serie)
    ...
    def get_chart_energy_price_in_dollar_kwh_without_production_taxes(self):
        ...
        for energy in energy_list:
            ...
            if self.stream_class_dict[energy].unit == "TWh":
                ...
                serie = InstanciatedSeries(
                    energy_prices[GlossaryEnergy.Years].values.tolist(),
                    techno_price[f"{energy}_wotaxes"].values.tolist(),
                    energy,
                    "lines",
                )
                new_chart.series.append(serie)
    ...
    def get_chart_energy_price_after_co2_tax_in_dollar_kwh(self):
        energy_prices_after_tax = self.get_sosdisc_outputs("energy_prices_after_tax")

        chart_name = (
            "Detailed prices of energy mix after carbon taxes due to combustion"
        )
            ...
            if energy != GlossaryEnergy.Years:
                if self.stream_class_dict[energy].unit == "TWh":
                    serie = InstanciatedSeries(
                        ...
                        energy_prices_after_tax[energy].values.tolist(),
                        energy,
                        "lines",
                    )
                    new_chart.series.append(serie)
    ...
    def get_chart_energy_price_in_dollar_t(self):
        ...
        for ccs_name in ccs_list:
            ...
            serie = InstanciatedSeries(
                energy_prices[GlossaryEnergy.Years].values.tolist(),
                techno_price[ccs_name].values.tolist(),
                ccs_name,
                "lines",
            )
            new_chart.series.append(serie)
    ...
    def get_chart_energy_mean_price_in_dollar_mwh(self):
        ...
        serie = InstanciatedSeries(
            ...
            energy_mean_price[GlossaryEnergy.EnergyPriceValue].values.tolist(),
            "mean energy",
            "lines",
        )
        new_chart.series.append(serie)
    ...
    def get_chart_co2_emissions_by_energy(self):
        ...
        for energy in co2_emissions:
            if energy != GlossaryEnergy.Years:
                ...
                serie = InstanciatedSeries(
                    x_serie_1, y_serie_1, energy, display_type="bar"
                )
    ...
    def get_chart_energies_net_raw_production_and_limit(self):
        ...
        minimum_energy_production = self.get_sosdisc_inputs("minimum_energy_production")
        chart_name = "Energy Total Production and Minimum Net Energy Limit"
        new_chart = TwoAxesInstanciatedChart(
            GlossaryEnergy.Years, "Energy [TWh]", chart_name=chart_name
        )

        energy_prod_raw = self.get_sosdisc_outputs("energy_production_brut")
        serie = InstanciatedSeries(
            energy_prod_raw[GlossaryEnergy.Years].values.tolist(),
            list(energy_prod_raw[GlossaryEnergy.TotalProductionValue].values.tolist()),
            "Raw Production",
            "lines",
        )
    ...
    def get_chart_energies_brut_production(self):
        ...
        for reactant in energy_production_detailed.columns:
            if (
                reactant
                not in [GlossaryEnergy.Years, GlossaryEnergy.TotalProductionValue]
                and GlossaryEnergy.carbon_capture not in reactant
                and GlossaryEnergy.carbon_storage not in reactant
            ):
                energy_twh = energy_production_detailed[reactant].values
    ...
    def get_pie_charts_production(self, years_list):
        ...
        techno_production = pd.DataFrame(
            {
                GlossaryEnergy.Years: energy_production_detailed[
                    GlossaryEnergy.Years
                ].values
            }
        )
        ...
        for energy in energy_list:
            if self.stream_class_dict[energy].unit == "TWh":
                techno_title = [
                    col
                    for col in energy_production_detailed
                    if col.endswith(f"{energy} ({GlossaryEnergy.energy_unit})")
                ]
                techno_production.loc[:, energy] = energy_production_detailed[
                    techno_title[0]
                ]
        ...
        for year in years_list:
            ...
            values = [
                ...
                for energy in energy_pie_chart
            ]
    ...
    def get_chart_co2_streams(self):
        ...
        serie = InstanciatedSeries(
            x_serie_1,
            (co2_emissions["CO2 emissions sources"].values / 1.0e3).tolist(),
        ...
        serie = InstanciatedSeries(
            x_serie_1,
            (co2_emissions["CO2 emissions sinks"].values / 1.0e3).tolist(),
            "CO2 emissions sinks",
        )
        new_chart.add_series(serie)
        ...
        serie = InstanciatedSeries(
            x_serie_1,
            (co2_emissions["Total CO2 emissions"].values / 1.0e3).tolist(),
            "CO2 emissions after CCUS",
        )
        new_chart.add_series(serie)
    ...
    def get_chart_co2_to_store(self):
        ...
        serie = InstanciatedSeries(
            ...
            (
                -co2_emissions[
                    f"{GlossaryEnergy.carbon_capture} needed by energy mix (Mt)"
                ].values
                / 1.0e3
            ).tolist(),
            f"{GlossaryEnergy.carbon_capture} used by energy mix",
            "bar",
        )
        new_chart.add_series(serie)
        ...
        serie = InstanciatedSeries(
            x_serie_1,
            (
                -co2_emissions[f"{GlossaryEnergy.carbon_capture} for food (Mt)"].values
                / 1.0e3
            ).tolist(),
            f"{GlossaryEnergy.carbon_capture} used for food",
            "bar",
        )
        new_chart.add_series(serie)
        ...
        serie = InstanciatedSeries(
            ...
            (
                co2_emissions[
                    f"{GlossaryEnergy.carbon_capture} to be stored (Mt)"
                ].values
                / 1.0e3
            ).tolist(),
            "CO2 captured to store",
        )
        new_chart.add_series(serie)
    ...
    def get_chart_co2_limited_storage(self):
        ...
        serie = InstanciatedSeries(
            ...
            (
                co2_emissions[
                    f"{GlossaryEnergy.carbon_storage} Limited by capture (Mt)"
                ].values
                / 1.0e3
            ).tolist(),
    ...
    def get_chart_co2_emissions_sources(self):
        ...
        serie = InstanciatedSeries(
            x_serie_1,
            (co2_emissions["Total CO2 by use (Mt)"].values / 1.0e3).tolist(),
            "CO2 by use (net production burned)",
        )
        new_chart.add_series(serie)
        ...
        serie = InstanciatedSeries(
            ...
            (
                co2_emissions[
                    f"Total {CarbonCapture.flue_gas_name} ({GlossaryEnergy.mass_unit})"
                ].values
                / 1.0e3
            ).tolist(),
    ...
    def get_chart_co2_needed_by_energy_mix(self):
        ...
        serie = InstanciatedSeries(
            ...
            (
                -co2_emissions[
                    f"{GlossaryEnergy.carbon_capture} needed by energy mix (Gt)"
                ].values
            ).tolist(),
    ...
    def get_chart_stream_ratio(self):
        ...
        streams = [
            col
            for col in all_streams_demand_ratio.columns
            if col
            not in [
                GlossaryEnergy.Years,
            ]
        ...
        min_objective_energy_loc = all_streams_demand_ratio[streams].min().idxmin()
        min_objective_year_loc = all_streams_demand_ratio[GlossaryEnergy.Years][
            all_streams_demand_ratio[streams].idxmin()[min_objective_energy_loc]
        ]
    ...
    def get_chart_energy_mix_losses(self, energy_list):
        ...
        raw_prod = self.get_sosdisc_outputs("energy_production_brut")
        raw_prod_detailed = self.get_sosdisc_outputs("energy_production_brut_detailed")
        ...
        heat_losses = (
            heat_losses_percentage
            / 100.0
            * raw_prod[GlossaryEnergy.TotalProductionValue].values
        )
        new_chart = TwoAxesInstanciatedChart(
            GlossaryEnergy.Years, "Energy losses (TWh)", chart_name=chart_name
        )
            ...
            if percentage_loss_name in inputs_dict:
                ...
                if percentage != 0.0:
                    ...
                    serie = InstanciatedSeries(
                        years,
                        losses.tolist(),
                        f"Distribution Transmission and Transport losses for {energy}",
                    )
                    new_chart.add_series(serie)
    ...
    def get_chart_stream_consumed_by_techno(self):
        ...
        for stream in streams:
            technologies_list_namespace_list = (
            ...
            if len(technologies_list_namespace_list) != 0:
                technologies_list += self.ee.dm.get_data(
                    technologies_list_namespace_list[0]
                )["value"]
        techno_cons_dict = {}
        for techno in technologies_list:
            techno_disc = self.ee.dm.get_disciplines_with_name(
                self.ee.dm.get_all_namespaces_from_var_name(
                    f"{techno}.{GlossaryEnergy.TechnoProductionValue}"
                )[0][: -len(".{GlossaryEnergy.TechnoProductionValue}")]
            )[0]
            ...
            consummed_stream = [
                col.split(" ")[0]
                for col in cons_col
                if col not in [GlossaryEnergy.Years]
            ]
        ...
        new_chart = InstanciatedTable(table_name=chart_name, header=header, cells=cells)
    ...
    def get_chart_capital(self):
        ...
        serie = InstanciatedSeries(
            ...
            capital_df[GlossaryEnergy.Capital].values.tolist(),
            "Capital",
            "lines",
        )
        new_chart.series.append(serie)
        ...
        serie = InstanciatedSeries(
            capital_df[GlossaryEnergy.Years].values.tolist(),
            capital_df[GlossaryEnergy.NonUseCapital].values.tolist(),
            "Non used capital (Utilisation ratio or limiting ratio)",
            "bar",
        )
        new_chart.series.append(serie)
        return new_chart
