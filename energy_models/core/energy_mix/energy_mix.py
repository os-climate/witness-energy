...
class EnergyMix(BaseStream):
    ...
    def compute_CO2_emissions(self):
        ...
        '''
        self.total_co2_emissions[f'Total CO2 by use (Mt)'] = self.co2_production[[
        ...
        self.total_co2_emissions[f'{CO2.name} for food (Mt)'] = self.co2_for_food[
        ...
        if cc_to_be_stored.min() < 0:
            cc_needed = self.total_co2_emissions[
        ...
        # COmpute CO2 emissions in Gt
        # This dataframe will be coupled
        self.total_co2_emissions_Gt = pd.DataFrame(
            {'years': self.production['years'],
             'Total CO2 emissions': self.total_co2_emissions['Total CO2 emissions'].values / 1e3})
...
#             self.total_co2_emissions[f'{CO2.name} for food (Mt)'
        ...
        cc_to_be_stored = co2_emissions[f'{CarbonCapture.name} (Mt) from CC technos'].values + \
            co2_emissions[f'{CarbonCapture.name} from energy mix (Mt)'].values - \
            co2_emissions[f'{CarbonCapture.name} needed by energy mix (Mt)'].values -\
            co2_emissions[f'{CO2.name} for food (Mt)'].values
        # if the carbon to be stored is lower than zero that means that we need
        # more carbon capture for energy mix than the one created by CC technos
        # or upgrading biogas
        if cc_to_be_stored.min() < 0:
            cc_needed = co2_emissions[
                f'{CarbonCapture.name} needed by energy mix (Mt)'].values
...
#             self.total_co2_emissions[f'Total CO2 by use (Mt)']
        ...
        new_key = 'Total CO2 emissions'
        key_dep_tuple_list = [('CO2 emissions sources', 1.0),
                              ('CO2 emissions sinks', -1.0)]
        dtot_CO2_emissions = update_new_gradient(
            dtot_CO2_emissions, key_dep_tuple_list, new_key)
        # Total is sources minus sinks
#         self.total_co2_emissions['Total CO2 emissions'] = self.total_co2_emissions['CO2 emissions sources'] - \
#             self.total_co2_emissions['CO2 emissions sinks']
        ...
        '''
        CO2 emissions objective gradient
        '''
        years = self.production['years'].values
        delta_years = (years[-1] - years[0] + 1)

        tot_co2_emissions_sum = co2_emissions['Total CO2 emissions'].sum()
        tot_co2_emissions_0 = co2_emissions['Total CO2 emissions'][0]

        dtot_final_CO2_emissions = dtot_CO2_emissions.copy()
        for key, gradient in dtot_CO2_emissions.items():
            if key.startswith('Total CO2 emissions'):
                new_key = key.replace(
                    'Total CO2 emissions', 'CO2 emissions objective')

                mask = np.insert(np.zeros(len(years) - 1), 0, 1)
                dco2_emissions_objective_dproduction = (alpha * gradient /
