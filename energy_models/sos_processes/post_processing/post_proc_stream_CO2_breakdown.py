...
def get_techno_price_data(execution_engine, namespace):
    ...
    #print('initial_y_values', initial_y_values)
    trace1 = go.Bar(
        x=list(techno_price_data.keys()),
        y=initial_y_values,
        name=str(year_list[0]),
        # offset=-0.2
    )
    ...
    for i in range(len(year_list)):
        ...
        #print('initial_y_values', [list(techno_price_data.keys())])
        step = dict(
            method='update',
            args=[{
                'x': [list(techno_price_data.keys())],
                'y[0]': initial_y_values,
                'name': str(year_list[i])
            }],
    ...
    layout = go.Layout(
        ...
        xaxis=dict(title='Title of x-axis'),
        yaxis=dict(title='Title of y-axis'),
        sliders=sliders,
        barmode='group'
    )
    ...
    #     fig, chart_name='Dummy', default_title=True)
    # return new_chart
