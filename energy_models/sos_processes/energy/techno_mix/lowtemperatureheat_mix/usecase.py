...
class Study(EnergyMixStudyManager):
    ...
    def get_investments(self):
        ...
        if 'NaturalGasBoilerLowHeat' in self.technologies_list:
            invest_low_heat_mix_dict['NaturalGasBoilerLowHeat'] = [
                0.02, 5.0, 5.0, 5.0, 5.0, 5.0, 5.0, 5.0]

        if 'ElectricBoilerLowHeat' in self.technologies_list:
            invest_low_heat_mix_dict['ElectricBoilerLowHeat'] = [
        ...
        if self.bspline:
            invest_low_heat_mix_dict[GlossaryCore.Years] = self.years
    ...
    def setup_usecase(self):
        ...
        self.energy_carbon_emissions = pd.DataFrame(
            {GlossaryCore.Years: years, 'biomass_dry': - 0.64 / 4.86, 'electricity': 0.0, 'methane': 0.0, 'water': 0.0})
        ...
        values_dict = {f'{self.study_name}.{GlossaryCore.YearStart}': self.year_start,
                       ...
                       f'{self.study_name}.{energy_name}.NaturalGasBoilerLowHeat.{GlossaryCore.MarginValue}': self.margin,
                       f'{self.study_name}.{energy_name}.ElectricBoilerLowHeat.{GlossaryCore.MarginValue}': self.margin,
                       f'{self.study_name}.{energy_name}.HeatPumpLowHeat.{GlossaryCore.MarginValue}': self.margin,
                       f'{self.study_name}.{energy_name}.GeothermalLowHeat.{GlossaryCore.MarginValue}': self.margin,
                       f'{self.study_name}.{energy_name}.CHPLowHeat.{GlossaryCore.MarginValue}': self.margin,
                       f'{self.study_name}.{energy_name}.HydrogenBoilerLowHeat.{GlossaryCore.MarginValue}': self.margin,
