...
from energy_models.sos_processes.energy.techno_mix.syngas_mix.usecase import TECHNOLOGIES_LIST_FOR_OPT
...
class ProcessBuilder(EnergyProcessBuilder):
    ...
    _ontology_data = {
        'label': 'Energy Technology Mix - Syngas Mix',
        'description': '',
        'category': '',
        'version': '',
    }
    ...
    def get_builders(self):
        ...
        mods_dict = {}
        mods_dict[f'{energy_mix}.{syngas_name}'] = self.get_stream_disc_path(
            'energy_disciplines', 'Syngas')
        for techno_name in self.techno_list:
            mods_dict[f'{energy_mix}.{syngas_name}.{techno_name}'] = self.get_techno_disc_path(
                syngas_name, techno_name)

        builder_list = self.create_builder_list(mods_dict, ns_dict=ns_dict)
        if self.invest_discipline == INVEST_DISCIPLINE_OPTIONS[0]:
            mods_dict_invest = {f'{energy_mix}.{syngas_name}': 'energy_models.core.investments.disciplines.techno_invest_disc.InvestTechnoDiscipline',
                                }
