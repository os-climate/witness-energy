...
class ProcessBuilder(EnergyProcessBuilder):
    ...
    _ontology_data = {
        'label': 'Energy Technology Mix - Clean Energy Mix',
        'description': '',
        'category': '',
        'version': '',
    }
    ...
    def __init__(self, ee):
        ...
        self.techno_list = GlossaryEnergy.DEFAULT_COARSE_TECHNO_DICT[GlossaryEnergy.clean_energy]['value']

    def get_builders(self):
        ns_study = self.ee.study_name

        energy_mix = 'EnergyMix'
        ns_dict = {'ns_clean_energy': f'{ns_study}.{energy_mix}.{CleanEnergy.name}',
        ...
        mods_dict[f'{energy_mix}.{CleanEnergy.name}'] = self.get_stream_disc_path(
            'energy_disciplines', 'CleanEnergy')
        for techno_name in self.techno_list:
            sub_dir = None
            mods_dict[f'{energy_mix}.{CleanEnergy.name}.{techno_name}'] = self.get_techno_disc_path(
                CleanEnergy.name, techno_name, sub_dir)
