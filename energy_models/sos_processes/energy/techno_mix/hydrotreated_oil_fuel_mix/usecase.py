...
TECHNOLOGIES_LIST_DEV = ['HefaDecarboxylation', 'HefaDeoxygenation']
...
class Study(EnergyMixStudyManager):
    ...
    def get_investments(self):
        ...
        if 'HefaDeoxygenation' in self.technologies_list:
            invest_hefa_mix_dict['HefaDeoxygenation'] = np.ones(
                len(l_ctrl))
        if self.bspline:
            invest_hefa_mix_dict[GlossaryCore.Years] = self.years

            for techno in self.technologies_list:
                invest_hefa_mix_dict[techno], _ = self.invest_bspline(
                    invest_hefa_mix_dict[techno], len(self.years))

        hefa_mix_invest_df = pd.DataFrame(
            invest_hefa_mix_dict)

        return hefa_mix_invest_df
    ...
    def setup_usecase(self):
        ...
        self.invest_level = pd.DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.InvestValue: 350.0 / 1000})
        ...
        self.resources_price = pd.DataFrame(
            columns=[GlossaryCore.Years, 'CO2', 'water', 'natural_oil'])
        self.resources_price[GlossaryCore.Years] = years
        self.resources_price['CO2'] = np.linspace(50.0, 100.0, len(years))
        self.resources_price['natural_oil'] = np.ones(len(years)) * 0.5 * 1000
        # biomass_dry price in $/kg
        self.energy_carbon_emissions = pd.DataFrame(
            {GlossaryCore.Years: years, 'biomass_dry': - 0.64 / 4.86, 'biogas': - 0.05, 'solid_fuel': 0.64 / 4.86, 'electricity': 0.0, 'methane': 0.123 / 15.4, 'syngas': 0.0, 'hydrogen.gaseous_hydrogen': 0.0, 'crude oil': 0.02533})
        ...
        values_dict = {f'{self.study_name}.{GlossaryCore.YearStart}': self.year_start,
                       ...
                       f'{self.study_name}.{energy_name}.HefaDecarboxylation.{GlossaryCore.MarginValue}': self.margin,
                       f'{self.study_name}.{energy_name}.HefaDeoxygenation.{GlossaryCore.MarginValue}': self.margin,
