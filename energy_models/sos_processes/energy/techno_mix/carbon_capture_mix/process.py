...
class ProcessBuilder(EnergyProcessBuilder):
    ...
    def get_builders(self):
        ...
        ns_dict = {'ns_carbon_capture': f'{ns_study}.{self.ccus_name}.{carbon_capture_name}',
                   ...
                   'ns_flue_gas': f'{ns_study}.{self.ccus_name}.{carbon_capture_name}.{flue_gas_name}',
                   'ns_public': f'{ns_study}',
                   'ns_resource': f'{ns_study}.{energy_mix}'}
        mods_dict = {}
        mods_dict[f'{self.ccus_name}.{carbon_capture_name}'] = self.get_stream_disc_path(
            'carbon_disciplines', 'CarbonCapture')
        mods_dict[
            f'{self.ccus_name}.{carbon_capture_name}.{flue_gas_name}'] = 'energy_models.core.stream_type.carbon_disciplines.flue_gas_disc.FlueGasDiscipline'
        for full_techno_name in self.techno_list:
            list_dot = full_techno_name.split('.')
            sub_dir = list_dot[0]
            techno_name = list_dot[1]
            mods_dict[f'{self.ccus_name}.{carbon_capture_name}.{full_techno_name}'] = self.get_techno_disc_path(carbon_capture_name, techno_name, sub_dir)
