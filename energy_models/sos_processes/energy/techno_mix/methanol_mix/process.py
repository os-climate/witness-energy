...
from energy_models.sos_processes.energy.techno_mix.methanol_mix.usecase import TECHNOLOGIES_LIST
...
class ProcessBuilder(EnergyProcessBuilder):
    ...
    _ontology_data = {
        'label': 'Energy Technology Mix - Methanol Mix',
        'description': '',
        'category': '',
        'version': '',
    }
    ...
    def get_builders(self):
        ...
        methanol_name = Methanol.name
        energy_mix = 'EnergyMix'
        ns_dict = {'ns_methanol': f'{ns_study}.{energy_mix}.{methanol_name}',
                   'ns_energy': f'{ns_study}.{energy_mix}',
                   'ns_energy_study': f'{ns_study}',
                   'ns_public': f'{ns_study}', 'ns_resource': f'{ns_study}.{energy_mix}'}

        mods_dict = {}
        mods_dict[f'{energy_mix}.{methanol_name}'] = self.get_stream_disc_path(
            'energy_disciplines', 'Methanol')
        for techno_name in self.techno_list:
            mods_dict[f'{energy_mix}.{methanol_name}.{techno_name}'] = self.get_techno_disc_path(
                Methanol.short_name, techno_name)

        builder_list = self.create_builder_list(mods_dict, ns_dict=ns_dict)
        if self.invest_discipline == INVEST_DISCIPLINE_OPTIONS[0]:
            mods_dict_invest = {f'{energy_mix}.{methanol_name}': 'energy_models.core.investments.disciplines.techno_invest_disc.InvestTechnoDiscipline',
                                }
