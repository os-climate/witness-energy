...
class Study(EnergyMixStudyManager):
    ...
    def get_investments(self):
        ...
        if 'AnaerobicDigestion' in self.technologies_list:
            invest_biogas_mix_dict['AnaerobicDigestion'] = np.ones(
                len(l_ctrl))
        if self.bspline:
            invest_biogas_mix_dict[GlossaryCore.Years] = self.years
    ...
    def setup_usecase(self):
        ...
        self.transport = pd.DataFrame(
            {GlossaryCore.Years: years, 'transport': 200.0})
        ...
        values_dict = {f'{self.study_name}.{GlossaryCore.YearStart}': self.year_start,
                       ...
                       f'{self.study_name}.{energy_name}.AnaerobicDigestion.{GlossaryCore.MarginValue}': self.margin,
